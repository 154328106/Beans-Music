...
- 新增 fnOS 飞牛音乐直连接入：服务器登录、曲目、专辑、歌单、收藏与搜索。
- 飞牛音乐封面、播放流、歌词与下载统一携带 `music-token` 认证，支持访问安全码与中继 Cookie。
- “发现 / 搜索 / 音乐库 / 我的”增加独立飞牛音乐入口，并保留原有 Subsonic、网易云、QQ 与酷狗逻辑。
- 增加局域网访问用途声明，便于连接 NAS 内网地址。

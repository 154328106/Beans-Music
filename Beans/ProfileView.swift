...
struct ProfileView: View {
    ...
    var body: some View {
        ...
        .task(id: auth.isLoggedIn) {
            ...
            await kugouAuth.fetchVIPStatus()
        }
    ...
    private var featuresGrid: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 12) {
                ...
                featureCell(icon: qqAuth.isLoggedIn || auth.isLoggedIn || kugouAuth.isLoggedIn ? "checkmark.seal.fill" : "globe", title: "账号与登录", subtitle: qqAuth.isLoggedIn || auth.isLoggedIn || kugouAuth.isLoggedIn ? accountStatusLine : "统一登录网易云 / QQ / 酷狗音乐") {
                    BeansHaptics.tap()
                    showAccountHub = true
                }
            }
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        qqCard
                        kugouCard
                        Text("网易云、QQ 音乐、酷狗音乐登录后可同步各平台歌单并提升可播成功率")
                            .font(BeansFont.appFont(11))
                            .foregroundStyle(Color.beansComment)
